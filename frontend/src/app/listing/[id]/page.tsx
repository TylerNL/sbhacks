...
import { Heart, Share2, Shield, Truck, ArrowLeft, ExternalLink, Loader2 } from 'lucide-react';
...
export default function ListingPage() {
    ...
    const checkIfSaved = async () => {
      ...
      try {
        ...
        if (!error && data?.saved) {
          ...
          setIsSaved(data.saved.includes(listingId));
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Handle null saved array
      let currentSaved: number[] = userData?.saved ?? [];
      let newSaved: number[];
      let nowSaved: boolean;

      if (currentSaved.includes(listingId)) {
        // Remove from saved
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          >
            ...
            <div>
              ...
              <p className="text-foreground/80 leading-relaxed">{listing.description || 'No description provided'}</p>
            </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="font-bold font-mono text-sm">
                    {listing.wallet_address.slice(0, 4)}...{listing.wallet_address.slice(-4)}
                  </p>
                </div>
                <a 
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className={`flex-1 py-4 border-2 font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors ${
                    isSaved ? 'border-accent text-accent' : 'border-border hover:border-foreground'
                  } disabled:opacity-50`}
                >
